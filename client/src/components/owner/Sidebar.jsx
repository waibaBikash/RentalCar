  ...
  return (
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 border-r border-borderColor text-sm'>
        ...
        <div className='group relative'>
           <label htmlFor="image">
             <img className='h-9 md:h-14 w-9 md:w-14 rounded-full' src={image ? URL.createObjectURL(image) : user ?.image || "https://unsplash.com/photos/a-white-car-parked-in-a-parking-lot-at-night-2u4Fa-htNU4"} alt="" />
         ...
         { image && (
           <button onClick={updateImage} className='absolute top-0 right-0 flex p-2 gap-1 bg-primary/10 cursor-pointer'>Save <img src={assets.check_icon} alt="" width={13} /></button>
         )}
